...
const setStatus=message=>{status.textContent=message;const text=String(message||'').toLowerCase();status.dataset.statusState=/error|failed|not supported/.test(text)?'error':/ready|download/.test(text)?'success':/decoding|processing|reading|encoding/.test(text)?'processing':'idle';};
let files=[];
...
mergeBtn.addEventListener('click',async()=>{
  if(files.length===0){setStatus('Add files first');return}
  setStatus('Decoding files...');try{
    ...
    const wav=encodeWAV(outBuf); const url=URL.createObjectURL(wav); downloadLink.href=url; downloadLink.download='merged.wav'; downloadLink.style.display='inline-block'; setStatus('Ready - download merged file.');
