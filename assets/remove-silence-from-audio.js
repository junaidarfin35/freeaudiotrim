...
const setStatus=message=>{status.textContent=message;const text=String(message||'').toLowerCase();status.dataset.statusState=/error|failed|not supported|no audible/.test(text)?'error':/ready|download/.test(text)?'success':/decoding|processing|reading|merging/.test(text)?'processing':'idle';};
...
processBtn.addEventListener('click',async()=>{
  const f=fileInput.files[0]; if(!f){setStatus('Choose a file');return}
  setStatus('Decoding...'); try{const abuf=await readFile(f);const actx=new (window.AudioContext||window.webkitAudioContext)();const decoded=await actx.decodeAudioData(abuf);
    ...
    if(merged.length===0){setStatus('No audible segments found with current settings.');return}
    // build new buffer
    ...
    const wav=encodeWAV(outBuf); const url=URL.createObjectURL(wav); downloadLink.href=url; downloadLink.download=(f.name.replace(/\.[^/.]+$/,'' )||'clean')+'_nosilence.wav'; downloadLink.style.display='inline-block'; setStatus('Ready - download cleaned file.');
  }catch(e){console.error(e);setStatus('This audio format is not supported by your browser. '+BROWSER_SUPPORT_MESSAGE);}}
, false);
